...
class App extends React.Component {
  constructor(props) {
    ...
    this.toTimeSting = this.toTimeSting.bind(this);
    this.toDayAndDate = this.toDayAndDate.bind(this);
    this.wheather = this.wheather.bind(this);
    this.wheatherIcon = this.wheatherIcon.bind(this);
  }
  componentDidMount() {
  ...
  wheatherIcon(stateWheather, intTime) {
    ...
    switch(true) {
      case stateWheather >= 200 && stateWheather < 300:
        return <i className="icon-clouds-flash-inv"></i>
      case stateWheather >= 300 && stateWheather < 600:
        return <i className="icon-rain-inv"></i>
      case stateWheather >= 600 && stateWheather < 700:
        return <i className="fa fa-snowflake-o"></i>
      case stateWheather >= 700 && stateWheather < 800:
        return <i className="icon-mist"></i>
      case stateWheather === 800:
        return d.getHours() < 19 && d.getHours() > 4 ? <i className="icon-sun-inv"></i> : <i className="icon-moon-inv"></i>
      case stateWheather === 801:
        return d.getHours() < 19 && d.getHours() > 4 ? <i className="icon-cloud-sun-inv"></i> : <i className="icon-cloud-moon-inv"></i>
      case stateWheather > 801 && stateWheather < 900:
        return <i className="icon-cloud-inv"></i>
      default:
        return <div>N/A</div>
    }
  }
  ...
  render () {
    if(this.props.wheather.pending) return <LoadSpiner />;
    return (
      Object.keys(this.props.wheather.data).length && !this.props.cityState ?
      <div>
           <div className="city-name">
            {this.props.wheather.data.city.name}, {this.props.wheather.data.city.country}
            <button className="change-btn" onClick={this.changeCity.bind(this)} >CHANGE</button>
           </div>
          <WheatherTop 
            wheather={this.props.wheather.data} 
          ...
          <div className="wheather-contatiner slider">
            <Slider {...this.state.sliderSettings}>
              {this.props.wheather.data.list.map(this.wheather)}
            </Slider>
          </div>
      </div>
      :
